...
tags: ["query:rl-ts"]
score: 7.0
evidence: 用于稳定离策SFT的强化学习重写代理
tldr: 针对大语言模型在下游任务微调（SFT）时因分布偏移导致的灾难性遗忘问题，本文提出了一种基于强化学习的数据重写代理。该方法将数据重写视为策略学习问题，通过RL优化重写分布，使其在保持任务一致性的同时，更好地匹配模型的自然生成分布并提升数据多样性。实验证明，该方法在保持下游任务性能的同时，显著缓解了非下游任务上的遗忘现象。
selection_source: fresh_fetch
motivation: 传统的监督微调（SFT）在面对分布偏移较大的下游数据时，容易导致模型产生严重的灾难性遗忘。
method: 提出一种基于强化学习的数据重写代理，通过奖励反馈联合优化QA风格的分布对齐、数据多样性及任务一致性。
result: "该方法在获得与标准SFT相当的下游任务增益的同时，将非下游基准测试中的遗忘率平均降低了12.34%。"
conclusion: 通过强化学习优化数据重写分布，可以有效弥合训练数据与模型分布间的差距，实现更稳定的离策监督微调。
---
