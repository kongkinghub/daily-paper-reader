...
tags: ["query:rl-ts"]
score: 7.0
evidence: 利用情节记忆防止失败的强化学习控制
tldr: "针对强化学习在复杂动力学环境下因频繁碰撞或跌落导致训练效率低下的问题，本文提出了 FEMA（失败情节记忆预警）方法。该方法通过情节记忆模块显式存储失败经验，在交互过程中检索相似的失败状态并发出预警，从而防止机器人重复陷入不稳定状态。FEMA 可与多种无模型强化学习算法结合，在 MuJoCo 任务中提升了 33.11% 的采样效率，并在真实双足机器人任务中验证了其有效性。"
selection_source: fresh_fetch
motivation: 强化学习早期训练常被碰撞和跌落等过早终止的短时域轨迹主导，严重阻碍了算法的收敛和长时域探索。
method: 提出 FEMA 框架，利用情节记忆模块存储失败经验，并在交互中实时检索相似状态以引导策略避开高风险区域。
result: "在 MuJoCo 基准测试中实现了 33.11% 的采样效率提升，并成功部署于真实双足机器人的并行训练流程中。"
conclusion: 通过显式学习和避开失败经验，FEMA 显著增强了强化学习在处理具有挑战性的接触动力学任务时的性能。
---
