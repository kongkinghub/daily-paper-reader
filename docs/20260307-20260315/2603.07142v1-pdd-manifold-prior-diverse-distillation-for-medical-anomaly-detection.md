...
## 摘要
医学图像异常检测由于嵌入在复杂解剖结构中的细微、异质异常而面临独特挑战。通过系统的 Grad-CAM 分析，我们揭示了判别性激活图在医学数据上失效，这与它们在工业数据集上的成功形成对比，从而激发了对流形级建模的需求。我们提出了 PDD（流形先验多样性蒸馏），这是一个将双教师先验统一到共享高维流形中，并将此知识蒸馏给具有互补行为的双学生的框架。具体而言，冻结的 VMamba-Tiny 和 wide-ResNet50 编码器分别提供全局上下文和局部结构先验。它们的特征通过流形匹配与统一（MMU）模块进行统一，而层间特征自适应（InA）模块则丰富了中间表示。统一的流形被蒸馏给两个学生：一个通过 InA 进行逐层蒸馏以实现局部一致性，另一个通过流形先验仿射（MPA）模块接收跳跃投影表示以捕获跨层依赖关系。多样性损失在保持检测敏感性的同时防止了表示崩溃。在多个医学数据集上的广泛实验表明，PDD 显著优于现有的最先进方法，在 HeadCT、BrainMRI 和 ZhangLab 数据集上的 AUROC 分别提升了高达 11.8%、5.1% 和 8.5%，在 Uni-Medical 数据集上的 F1 max 提升了 3.4%，在医学图像异常检测中确立了新的最先进性能。实现代码将发布在 https://github.com/OxygenLu/PDD。
