...
tags: ["query:rl-ts"]
score: 7.0
evidence: 消除直接 Q 函数估计以避免偏差并提高稳定性
...
## 摘要
在本文中，我们提出了一种新型多智能体强化学习框架，通过准确的单智能体优势估计来提高样本效率和协调性。我们方法的核心是广义单智能体优势估计器（GPAE），它采用单智能体价值迭代算子来计算精确的单智能体优势。该算子通过动作概率间接估计价值，从而实现稳定的离策（off-policy）学习，消除了对直接 Q 函数估计的需求。为了进一步优化估计，我们引入了一种双重截断重要性采样比率方案。该方案通过平衡对智能体自身策略变化的敏感性与对其他智能体非平稳性的鲁棒性，改进了离策轨迹的信用分配。基准测试实验表明，我们的方法优于现有方法，在复杂场景的协调性和样本效率方面表现出色。
